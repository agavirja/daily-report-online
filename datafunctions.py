...
def propertydata():
    ...
    datacaracteristicas = pd.read_sql("SELECT * FROM colombia.data_stock_inmuebles_caracteristicas", con=db_connection)
    dataimagenes        = pd.read_sql("SELECT * FROM colombia.data_stock_inmuebles_img", con=db_connection)
...
def propertymanagementdata():
    db_connection = sql.connect(user=user, password=password, host=host, database=schema)
    data          = pd.read_sql("SELECT * FROM colombia.app_pm_cuentas", con=db_connection)
...
def followup():
    db_connection = sql.connect(user=user, password=password, host=host, database=schema)
    data          = pd.read_sql("SELECT * FROM colombia.app_callcenter_callhistory_inbound", con=db_connection) 
    return data

@st.experimental_memo
def inspeccion_callsample():
    db_connection = sql.connect(user=user, password=password, host=host, database=schema)
    data          = pd.read_sql("SELECT * FROM colombia.app_callsample", con=db_connection) 
...
def inspeccion_visitas():
    ...
    datacallhistory  = pd.read_sql("SELECT * FROM colombia.app_callhistory;", con=db_connection) 

    # Caracteristicas
    datacaracteristicas = pd.read_sql("SELECT * FROM colombia.app_inspeccion_caracteristicas;", con=db_connection) 
    # Fotos
    datafotos    = pd.read_sql("SELECT * FROM colombia.app_seguimiento_fotos_visitas", con=db_connection)
    # Estado del ambiente 
    dataambiente = pd.read_sql("SELECT * FROM colombia.app_inspection_ambiente", con=db_connection) 
...
def inspeccion_callhistory():
    db_connection = sql.connect(user=user, password=password, host=host, database=schema)
    data          = pd.read_sql("SELECT * FROM colombia.app_callhistory", con=db_connection)
...
def inspeccion_seguimiento_ofertas():
    db_connection = sql.connect(user=user, password=password, host=host, database=schema)
    data          = pd.read_sql("SELECT * FROM colombia.data_stock_inmuebles_ofertas", con=db_connection)
    return data  
...
def inspeccion_seguimiento_ofertas_merge():
    ...
    datacaracteristicas = pd.read_sql(f"SELECT * FROM colombia.app_inspeccion_caracteristicas WHERE {idlist}", con=db_connection)
...
def datadocumentos():
    db_connection = sql.connect(user=user, password=password, host=host, database=schema)
    data          = pd.read_sql("SELECT * FROM colombia.data_stock_inmuebles_documents", con=db_connection) 
    return data

@st.experimental_memo
def datarecorridos():
    db_connection = sql.connect(user=user, password=password, host=host, database=schema)
    data          = pd.read_sql("SELECT fecha_recorrido,nombre_conjunto,direccion_formato, direccion_porteria, upzcodigo,scanombre,tipo_negocio,telefono1, telefono2,telefono3, latitud,longitud,url_foto_fachada,url_foto_direccion,status,coddir FROM colombia.app_recorredor_stock_ventanas", con=db_connection) 
    return data
